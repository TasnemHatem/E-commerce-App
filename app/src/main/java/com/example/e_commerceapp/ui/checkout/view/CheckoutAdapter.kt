...
class CheckoutAdapter (
    var list: List<LineItemsItem>, ) : RecyclerView.Adapter<CheckoutAdapter.CartViewHolder>() {
    ...
    class CartViewHolder(
        private val mItem: CheckoutItemBinding, val list: List<LineItemsItem>) :
        RecyclerView.ViewHolder(mItem.root) {
        @SuppressLint("SetTextI18n")
        fun bind(item: LineItemsItem, position: Int, ) {
            mItem.tvCartItemPrice.text =
